DELETE FROM action.report
WHERE reporttypeFK = 'ACTIONPLANS';


DELETE FROM action.reporttype
WHERE reporttypePK = 'ACTIONPLANS';
...
BEGIN
    ...
    PERFORM action.logmessage(p_messagetext := 'GENERATING ACTION MI REPORT'
       ...
       v_rows     := 0;
       v_contents := '';
       v_contents := 'Action Plan No,Action Plan Name,Action Type,Action Plan Start Date,Days Off Set,Handler,Count,State';
